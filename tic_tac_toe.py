...
class Computer():
    ...
    def return_random_spot(self, board):
        coors = [random.randint(0, 2), random.randint(0, 2)]
...
def player_vs_computer_normal(board):
    running = True
    ...
    while running:
        if (not p1.win or not computer.win) and running:
            ...
            if not computer.win and running:
                running = board_update(board, p1.option(), p1)
                p1.win, running = win_check(board, p1)
                if running:
                    running = not_full_board_check(board)
...
def player_vs_computer_easy(board):
    running = True
    ...
    while running:
        if (not p1.win or not computer.win) and running:
            board = board_update_cpu(board, computer.return_random_spot(board), computer)
            computer.win, running = win_check(board, computer)
            print_board(board)
            if running:
                running = not_full_board_check(board)
...
def difficulty(board):
    ...
    if option == "1":
        ...
        player_vs_computer_normal(board)    
